...
class DatePicker extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this.calendarDaysContainer.addEventListener("click", (event) => {
      const selectedDayElement = event.target.closest(".month-day");
      if (selectedDayElement) {
        const selectedDayIndex = Array.from(
          this.calendarDaysContainer.children
        ).indexOf(selectedDayElement);
        const selectedDay = this.getMonthDaysGrid()[selectedDayIndex];

        // Now 'selectedDay' contains the Day object for the selected date
        // selectedDay.date
        // selectedDay.monthNumber
        // selectedDay.year
        let demo = `${selectedDay.year}-${
          selectedDay.date < 10 ? "0" + selectedDay.date : selectedDay.date
        }-${
          selectedDay.monthNumber < 10
            ? "0" + selectedDay.monthNumber
            : selectedDay.monthNumber
        } `;

        const noPalit = document.createElement("input");
        noPalit.classList.add("noPalit")
        noPalit.setAttribute("type", "hidden");
        noPalit.value = demo;
        document.body.append(noPalit);
     

        // You can also update your component's state or trigger custom logic here
      }
    });
